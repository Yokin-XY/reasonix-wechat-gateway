...
async def main() -> None:
    ...
    system_append = args.prompt_suffix or (
        ...
        "工作目录规范：工作台位于 /workspace/.kf/toolchains/node-v24.15.0/lib/node_modules/reasonix/workspace/projects/。"
        "项目源码和配置文件统一放在该目录下管理。"
        "但构建、编译等文件密集型操作必须先在 /root/ 目录下创建副本再执行，"
        "禁止在 /workspace/、/exchange/、/chuan/、/xiangce/ 等共享目录下直接执行文件密集型操作。"
        "共享目录只用于文件交换，不适合大量读写。"
    )
    acp_config = AcpConfig(
        dir=args.dir,
